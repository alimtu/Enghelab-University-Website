...
import { CircleX, CirclePlay, PlayCircleIcon } from 'lucide-react';
...
export default function IntroductionOfDeputyComponent({ data = DefaultData }) {
  ...
  return (
    <div className="w-full bg-white flex flex-col lg:flex-row justify-center items-start gap-4 lg:gap-8 px-4 lg:px-20 ">
      <div className="w-full lg:w-1/2 xxl:w-1/2 flex flex-col gap-2 lg:gap-4 xxl:gap-8 justify-center items-centere h-full">
        <h2 className="font-bold text-2xl lg:text-4xl xxl:text-5xl">{data.title}</h2>
        <p className="flex-1 text-sm lg:text-base  text-grey-800 xxl:text-lg leading-6 lg:leading-8 xxl:leading-10 text-justify wrap-anywhere">
          {data.description}
        </p>
      </div>
      <div className="w-full lg:w-1/2 xxl:w-1/2  relative  flex items-stretch cursor-pointer">
        <div className="relative w-full aspect-video rounded-lg overflow-hidden h-full">
          <Image
            src={data.imagePath}
            alt="Video thumbnail"
            className="h-auto object-cover lg:w-1/2 w-full"
            fill
            priority
          />
