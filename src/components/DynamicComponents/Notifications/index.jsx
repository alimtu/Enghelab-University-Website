...
export default function NotificationsComponent({ data }) {
  return (
    <div className="relative w-full pr-4 lg:px-20  flex flex-col gap-6">
      <div className="flex flex-row items-center justify-center lg:justify-between lg:text-right">
        <h3 className="text-[#101828] font-medium text-2xl lg:text-3xl">{data.title}</h3>
      ...
      >
        {data.items.map((item, index) => (
          <SwiperSlide
            key={`item_${index}`}
            className="h-full flex flex-col pb-4 border rounded-2xl border-neutral-100 "
          >
            ...
            <div className="flex flex-col px-4 h-full lg:min-h-[224px] lg:max-h-[224px] min-h-[199px] max-h-[199px]">
              ...
              <div className="text text-[#252B37] text-opacity-75 mb-3 flex-grow">
                <p className="text-sm leading-7 line-clamp-3">{item.description}</p>
              </div>
